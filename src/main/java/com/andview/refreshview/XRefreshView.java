    ...
    public void stopLoadMore(final boolean hideFooter) {
        if (needAddFooterView()) {
            if (mPullLoading == true) {
                ...
                if (mPinnedTime >= 200) {// 在加载更多完成以后，只有mPinnedTime大于1s才生效，不然效果不好
                    postDelayed(new Runnable() {
                        @Override
                        public void run() {
...
//                mFooterCallBack.show(true);
    ...
    private void endLoadMore(boolean hideFooter) {
        mPullLoading = false;
        startScroll(-mHolder.mOffsetY, mPinnedTime);
//        mFooterCallBack.onStateRefreshing();
    ...
    public <T> T getCustomHeaderView(Class<T> c) {
    ...
    public <T> T getCustomFooterView(Class<T> c) {
        return (T) mFooterView;
    }
    ...
    public void notifyDataSetChanged() {
        ...
        if (element.toView() instanceof AbsListView) {
            if (element.getListViewAdapter() != null) {
                if (element.getListViewAdapter() instanceof BaseAdapter) {
                    ((BaseAdapter) element.getListViewAdapter()).notifyDataSetChanged();
                }
            }
