...
import com.wheel.base.WeBaseView;
 ...
 * @since 2013-12-26
 */
public class WeScrollableView extends WeBaseView {
    private static final int MSG_SCROLL_STEP = 1;
    private static final int MSG_SCROLL_END = 2;
    /**
     * 滚动状态
     */
    protected WeScrollState mScrollState;
    /**
     * 动作步列表
     */
    private LinkedList<WeStep> mStepList;
    ...
    public WeScrollableView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    public WeScrollableView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public WeScrollableView(Context context) {
        super(context);
    }
    ...
    protected void onInit(Context context) {
        ...
        mStepList = new LinkedList<WeStep>();
        mScrollState = new WeScrollState(context);
    }
     ...
     * 设置动画变化率
     *
     * @param interpolator 变化率算法对象
     */
    protected void setInterpolator(Interpolator interpolator) {
        mScrollState = new WeScrollState(this.getContext(), interpolator);
    }
    ...
    public void smoothScrollTo(int finalX, int finalY) {
        smoothScrollTo(finalX, finalY, WeStep.DEFAULT_DURATION);
    }
     ...
     * 屏幕左上角为坐标原点，水平向右→为X轴正向，垂直向下↓为Y轴正向
     *
     * @param distanceX distanceX>0向右移动
     * @param distanceY distanceY>0向下移动
     */
    public void smoothScrollBy(int distanceX, int distanceY) {
        /* 先停止当前的滚动 */
        stopScroll();
        /* 设置滚动参数 */
        mScrollState.setScrollParams(distanceX, distanceY, WeStep.DEFAULT_DURATION);
        /* 启动滚动 */
        startScroll();
    }
     ...
     * @param stepList
     */
    public void move(LinkedList<WeStep> stepList) {
        stopCurrentMovement();
        mStepList.clear();
    ...
    private void actStep() {
        if (mStepList == null
                || mStepList.isEmpty())
            return;
        WeStep step = mStepList.removeFirst();
    ...
    private void scrollDistanceStep(WeStep step) {
        smoothScrollBy(step.getDistanceX(), step.getDistanceY(), step.getDuration());
    }

    private void scrollFinalStep(WeStep step) {
